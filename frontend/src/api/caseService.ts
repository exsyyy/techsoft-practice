...
export const caseService = {
    ...
    create: async (
        caseData: Omit<CaseStudy, 'id' | 'status' | 'created_at' | 'updated_at' | 'technologies' | 'verifier_id' | 'verification_date'>
    ): Promise<CaseStudy> => {
        const response = await axiosClient.post<CaseStudy>('/api/cases/admin/cases', caseData)
        return response.data
    },
